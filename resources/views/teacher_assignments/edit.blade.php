...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Sửa Phân Công Giáo Viên</h1>
    @if (session('success'))
    ...
    <form action="{{ route('teacher_assignments.update', $teacherAssignment) }}" method="POST" class="bg-white p-6 rounded shadow">
        @csrf @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="class_id" class="w-full border rounded p-2" required>
                ...
                @foreach($classes as $class)
                    <option value="{{ $class->id }}" {{ old('class_id', $teacherAssignment->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                @endforeach
            </select>
            @error('class_id')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <select name="subject_id" class="w-full border rounded p-2" required>
                ...
                @foreach($subjects as $subject)
                    <option value="{{ $subject->id }}" {{ old('subject_id', $teacherAssignment->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                @endforeach
            </select>
            @error('subject_id')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <textarea name="note" class="w-full border rounded p-2" rows="2" maxlength="255">{{ old('note', $teacherAssignment->note) }}</textarea>
            @error('note')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Cập Nhật Phân Công</button>
        </div>
    </form>
